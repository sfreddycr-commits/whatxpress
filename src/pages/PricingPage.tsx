...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
          ...
          {(dbPlans.length > 0 ? dbPlans : [{id:"f1",name:"Starter",price:29,features:[],is_popular:false},{id:"f2",name:"Pro",price:99,features:[],is_popular:true}]).map((plan: any) => {
            ...
            <div key={plan.id||plan.name} className={"relative rounded-3xl border shadow-sm overflow-hidden flex flex-col " + (isPro ? "bg-[#109e38] border-[#109e38] text-white" : "bg-white border-slate-200")}>
              ...
              <div className="p-6 sm:p-8 flex-1 flex flex-col">
                ...
                <button onClick={() => nav("https://app.whatxpress.com/login")} className={"w-full h-12 rounded-xl font-black text-sm flex items-center justify-center gap-2 transition-all " + (isPro ? "bg-white text-[#109e38] hover:bg-green-50" : "bg-slate-900 text-white hover:bg-slate-800")}>{isPro ? tr.ctaTrial : tr.ctaStart} <ArrowRight className="w-4 h-4" /></button>
              </div>
            </div>
            );
          })}
      ...
      <section className="bg-[#109e38] py-16 px-4 text-center text-white"><h2 className="text-2xl sm:text-3xl font-black mb-3">{tr.cta.title}</h2><p className="text-green-100 mb-6 max-w-md mx-auto text-sm">{tr.cta.desc}</p><button onClick={() => nav("https://app.whatxpress.com/login")} className="inline-flex items-center gap-2 h-14 px-8 bg-white text-[#109e38] rounded-2xl font-black text-sm hover:bg-green-50 transition-all shadow-lg">{tr.cta.btn} <ArrowRight className="w-4 h-4" /></button></section>
